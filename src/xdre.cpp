...
//variable que sirve para que no se haga display en cada instrucción si se está creando una demo
...
ResultDemo demoDesdeLista(std::vector<std::string> list){
    //se pone a true la variable para que no haga display cada instrucción
    creandoDemo = true;
    ...
    for (i=0;i<list.size();i++){
        ...
        //si es el último tic, saca el tiempo y no hagas nuevos tics
        if (i==list.size()-1){
            tiempo = xdre::getTime();
...
void xdre::init() {
    ...
    if(completaNivel == false){
        ...
        cout << "Completa el nivel: Sí" << endl;
    }
...
void xdre::copyTic() {
    ...
    goForward(1);
    //comprueba que no se está creando una demo
